  ...
  return (
    ...
    >
      <header className="container-page-heading">
        <div className="container-title"><span className="container-glyph large"><span /></span><div><div><h1>{container?.name ?? containerId.slice(0, 12)}</h1>{container ? <span className={`status-badge ${container.status}`}><i />{container.status}</span> : null}</div><p>{container?.image ?? (loading ? "Loading container metadata" : "Container metadata unavailable")}</p></div></div>
        <div className="container-heading-actions">
      ...
      {error ? <div className="status-banner error" role="alert"><TriangleAlert size={18} /><div><strong>Container data interrupted</strong><span>{error}</span></div><button onClick={() => void load()} type="button">Retry</button></div> : null}
      ...
      {tab === "overview" ? <>
        <section className="operations-summary container-summary" aria-label="Container summary">
          <div><span>CPU</span><strong>{liveContainer?.stats.cpu_percent.toFixed(1) ?? "—"}%</strong><small>current utilization</small></div>
          <div><span>Memory</span><strong>{liveContainer ? formatBytes(liveContainer.stats.memory_usage) : "—"}</strong><small>{liveContainer ? `${liveContainer.stats.memory_percent.toFixed(1)}% of limit` : "current usage"}</small></div>
          <div><span>Processes</span><strong>{liveContainer?.stats.pids ?? "—"}</strong><small>container PIDs</small></div>
          <div><span>Restarts</span><strong>{container?.restart_count ?? "—"}</strong><small>{container?.health?.status ?? "no health check"}</small></div>
        </section>
        <div className="container-overview-grid">
          <section className="panel container-facts"><div className="panel-heading"><div><p className="eyebrow">Runtime</p><h2>Container state</h2></div></div><dl>
          ...
          <section className="panel container-history-panel"><div className="panel-heading"><div><p className="eyebrow">Last 24 hours</p><h2>Resource history</h2></div><span className="heading-count">{history.length} samples</span></div><div className="container-history-chart">{chartData.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={chartData} margin={{ top: 12, right: 18, left: -14, bottom: 0 }}><CartesianGrid stroke="#e8eaed" strokeDasharray="3 3" vertical={false} /><XAxis dataKey="label" minTickGap={36} tick={{ fontSize: 10, fill: "#717680" }} /><YAxis tick={{ fontSize: 10, fill: "#717680" }} unit="%" /><Tooltip /><Legend /><Line dataKey="cpu_percent" dot={false} isAnimationActive={false} name="CPU" stroke="#087f8c" strokeWidth={2} type="monotone" /><Line dataKey="memory_percent" dot={false} isAnimationActive={false} name="Memory" stroke="#7868b4" strokeWidth={2} type="monotone" /></LineChart></ResponsiveContainer> : <div className="chart-waiting">Waiting for retained samples</div>}</div></section>
        </div>
        <div className="container-network-grid">
          <section className="panel"><div className="panel-heading"><div><p className="eyebrow">Connectivity</p><h2>Networks and ports</h2></div></div><div className="detail-list">{container?.networks.map((network) => <div key={network.name}><Network size={16} /><div><strong>{network.name}</strong><small>{network.ip_address || "No IP assigned"}{network.gateway ? ` · gateway ${network.gateway}` : ""}</small></div></div>)}{!container?.networks.length ? <div className="detail-list-empty">No attached networks</div> : null}{container?.ports.map((port) => <div key={`${port.container_port}-${port.host_port}`}><Box size={16} /><div><strong>{port.container_port}</strong><small>{port.host_port ? `${port.host_ip}:${port.host_port}` : "Not published"}</small></div></div>)}</div></section>
          <section className="panel"><div className="panel-heading"><div><p className="eyebrow">Storage</p><h2>Mounts</h2></div></div><div className="mount-table">{container?.mounts.map((mount) => <div key={`${mount.source}-${mount.destination}`}><span className="state-label resolved">{mount.type}</span><div><strong>{mount.destination}</strong><small title={mount.source}>{mount.source}</small></div><span>{mount.rw ? "Read / write" : "Read only"}</span></div>)}{!container?.mounts.length ? <div className="detail-list-empty">No attached mounts</div> : null}</div></section>
        </div>
      </> : null}

      {tab === "inspect" ? <div className="inspect-grid">
        <section className="panel inspect-section"><div className="panel-heading"><div><p className="eyebrow">Configuration</p><h2>Environment</h2></div><span className="heading-count">sensitive values redacted</span></div><dl className="key-value-list">{container?.environment.map((item) => <div key={item.key}><dt>{item.key}</dt><dd className="mono">{item.value}</dd></div>)}{!container?.environment.length ? <div className="detail-list-empty">No environment entries</div> : null}</dl></section>
        <section className="panel inspect-section"><div className="panel-heading"><div><p className="eyebrow">Metadata</p><h2>Labels</h2></div></div><dl className="key-value-list">{Object.entries(container?.labels ?? {}).map(([key, value]) => <div key={key}><dt>{key}</dt><dd className="mono">{value}</dd></div>)}{!Object.keys(container?.labels ?? {}).length ? <div className="detail-list-empty">No labels</div> : null}</dl></section>
        <section className="panel inspect-section"><div className="panel-heading"><div><p className="eyebrow">Limits</p><h2>Resource configuration</h2></div></div><dl className="key-value-list">{Object.entries(container?.resources ?? {}).map(([key, value]) => <div key={key}><dt>{key.replaceAll("_", " ")}</dt><dd>{value || "Not limited"}</dd></div>)}</dl></section>
        <section className="panel inspect-section"><div className="panel-heading"><div><p className="eyebrow">Process</p><h2>Launch configuration</h2></div></div><dl className="key-value-list"><div><dt>Entrypoint</dt><dd className="mono">{container?.entrypoint.join(" ") || "Image default"}</dd></div><div><dt>Command</dt><dd className="mono">{container?.command.join(" ") || "Image default"}</dd></div><div><dt>Working directory</dt><dd>{container?.working_dir || "/"}</dd></div><div><dt>User</dt><dd>{container?.user || "Image default"}</dd></div></dl></section>
      </div> : null}

      {tab === "events" ? <section className="panel events-panel"><div className="table-toolbar"><label className="search-field"><Search size={16} /><span className="sr-only">Search events</span><input onChange={(event) => setEventSearch(event.target.value)} placeholder="Search events" type="search" value={eventSearch} /></label><div className="events-toolbar-state"><label className="compact-select"><span>Action</span><select onChange={(event) => setEventAction(event.target.value)} value={eventAction}><option value="all">All actions</option>{eventActions.map((action) => <option key={action} value={action}>{action}</option>)}</select></label><span className={eventsConnected ? "stream-state connected" : "stream-state"}><i />{eventsConnected ? "Live" : "Reconnecting"}</span></div></div><div className="events-list">{visibleEvents.map((event) => <div key={event.id}><span className="event-icon"><Activity size={15} /></span><div><strong>{event.action}</strong><small>{event.image ?? event.container_name}</small></div><time dateTime={new Date(event.timestamp * 1000).toISOString()}>{dateTime(event.timestamp)}</time></div>)}{!visibleEvents.length ? <div className="events-empty"><Activity size={23} /><strong>No matching Docker events</strong><span>Events will appear here as container state changes occur.</span></div> : null}</div></section> : null}
